...
| ビルド定義 | `Flow.Launcher.Plugin.Notion.csproj` |
...
コピー後に Flow Launcher を再起動し、**Settings > Plugins** に Notion が表示されることを確認します。

## リリース

GitHub Actions の **Release > Run workflow** を実行すると、Windows 上で依存関係の復元と Release ビルドを行い、`Flow.Launcher.Plugin.Notion.zip` を GitHub Release に添付します。
